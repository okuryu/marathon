...
class TaskTrackerTest extends MarathonSpec with Matchers with GivenWhenThen {
  ...
  test("ensure that determineOverdueTasks returns tasks disregarding the stagedAt property") {
    ...
    val unstagedTask = MarathonTask.newBuilder()
      .setId("unstaged")
      .build()
    assert(unstagedTask.getStagedAt == 0, "The stagedAt property of an unstaged task has a value of 0")
    assert(unstagedTask.getStartedAt == 0, "The startedAt property of an unstaged task has a value of 0")
    ...
    val stagedLongTimeAgoTask = MarathonTask.newBuilder()
      .setId("stagedLongTimeAgoTask")
    ...
    Given("several tasks of which not all have been staged yet")
    taskTracker.created(TEST_APP_NAME, unstagedTask)
    taskTracker.created(TEST_APP_NAME, stagedLongTimeAgoTask)
    ...
    Then("Both the unstaged and the stagedLongAgo tasks are returned")
    assert(stagedTasks == Seq(stagedLongTimeAgoTask, unstagedTask))
